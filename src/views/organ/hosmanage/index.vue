    ...
    >
      ...
      >
        ...
        <el-form-item label="管理手机号" prop="sendPhone">
          <div class="flex-start" v-for="(val, i) in phoneList" :key="i">
            <el-input
              placeholder="请输入"
              v-model="val.sendPhone"
              @input="changeInoo"
            ></el-input>
            <span v-if="i === 0" class="add-reduce" @click="addPhone">+</span>
            <span v-else class="add-reduce" @click="reducePhone(i)">-</span>
          </div>
...
export default {
  ...
  data() {
    var validatorSendPhone = (rule, value, callback) => {
      const reg = /^[1]([3-9])[0-9]{9}$/
      console.log(reg)
      if (!this.phoneList.every(v => v.sendPhone)) {
        return callback(new Error('请输入管理手机号'))
      }
      if (!this.phoneList.every(v => reg.test(v.sendPhone))) {
        return callback(new Error('请检查手机号格式'))
      }
      callback()
    }
    return {
  ...
  methods: {
      ...
      this.$refs.editForm.validate(async valid => {
        if (valid) {
          const hospType = this.switchStatus
            ...
            .map(v => v.value)
            .join(',')
          const sendPhone = this.phoneList.map(v => v.sendPhone).join(',')
          if (!this.dialogData.formData.telNo)
            delete this.dialogData.formData.telNo
          this.dialogData.formData.hospType = hospType
          this.dialogData.formData.sendPhone = sendPhone
    ...
    editHosp(row) {
      ...
      this.phoneList = row.sendPhone
        ? row.sendPhone.split(',').map(v => {
            return { sendPhone: v }
          })
        : [{ sendPhone: '' }]
      console.log(row, '当前行===')
