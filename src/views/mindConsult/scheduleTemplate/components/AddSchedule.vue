<template>
  <el-dialog
    :title="modelType === 'add' ? '添加排班内容' : '编辑排班内容'"
...
export default {
  ...
  watch: {
    show() {
      if (this.show) {
        //编辑状态下 剔除当前行的列表(放开当前行时段的选择限制)
        this.tableList =
          this.modelType === 'edit'
            ? this.list.filter(v => v.id !== this.rowData.id)
            : this.list
        this.getTime()
    ...
    rowData() {
      ...
      this.form = Object.assign(this.form, {
        ...
        ageGroup: this.rowData.ageGroup ? this.rowData.ageGroup.split(',') : [],
        sex: this.rowData.sex,
        video: this.rowData.video,
        sourceNumber: this.rowData.sourceNumber,
        detailId: this.rowData.id,
        weekDay: this.rowData.weekDay,
      })
  ...
  methods: {
    ...
    itemChange(e) {
      ...
      if (this.form.startTime) {
        ...
        //根据项目时长重新赋值满足条件的时段的开始时间————————————————————————————————————
        //筛选可用时段（>=所选项目需要时长的时段）
        const canUseTimes = rangge.data.filter(v => {
          const start = dayjs(`2020-01-01${v.startTime}:00`)
          const end = dayjs(`2020-01-01${v.endTime}:00`)
          return end - start >= item.duration * 60 * 1000
        })
        if (canUseTimes.length) {
          this.form.startTime = canUseTimes[0].startTime
        }
        //---------——————————————————————————————————————————————————————————----
    ...
    async getTime() {
      ...
      //编辑状态下 匹配午别选中项
      if (this.modelType === 'edit') {
        this.form.periodId = this.timeList.find(
          v => v.name === this.rowData.periodName,
        ).id
      }
    },
    //获取项目
      ...
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          ...
          if (this.modelType === 'add') {
            ...
            //编辑状态下
            await apiScheduleTemplate.editDoctorSchedule(this.form)
          }
          this.close(true)
