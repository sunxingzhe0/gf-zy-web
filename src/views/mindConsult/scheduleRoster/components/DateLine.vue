...
export default {
  ...
  methods: {
    ...
    next() {
      if (this.currentIndex < this.tiemDatas.length - 1) {
        let dateRef = this.$refs.dateRef
        let itemTop = this.$refs[`dateItem${this.currentIndex + 1}`][0]
          .offsetLeft
        if (itemTop > dateRef.offsetWidth - 120) {
          dateRef.scrollLeft = itemTop
        }
        console.log(dateRef.offsetWidth, dateRef.scrollLeft)
        this.$emit('onchange', this.tiemDatas[this.currentIndex + 1])
      }
    },
    prev() {
