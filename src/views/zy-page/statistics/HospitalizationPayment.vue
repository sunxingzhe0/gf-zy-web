...
import { zyPayData, exportZyPayData } from '@/api/zyapi/statistics'
...
export default {
  ...
  mixins: [mixin({ fetchListFunction: zyPayData })],
  data() {
    return {
      query: {
        pageSize: 10,
  ...
  methods: {
    handleCommand(command) {
      ...
      if (command == 'query') {
        exportZyPayData(this.query)
      }
      if (command == 'selection') {
        let select = []
        ids.forEach(i => [select.push(i.orderId)])

        exportZyPayData({ orderIds: select })
      }
    },
    goto(type, id, docId) {
      this.$router.push(
